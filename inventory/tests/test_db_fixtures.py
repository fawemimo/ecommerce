...
'''
This is to test the DB for category in REST FRAMEWORK
''' 
@pytest.fixture
def category_with_multiple_children(db):
    record = models.Category.objects.build_tree_nodes(
        {
            "id": 1,
            "name": "parent",
            "slug": "parent",
    ...
    category = models.Category.objects.bulk_create(record)
    return category
